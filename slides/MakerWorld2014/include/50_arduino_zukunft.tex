...
\begin{itemize}
\item Intels Versuch, mit Embedded x86 in die Nähe von Microcontrollern zu kommen
\item Referenzboard für Intels Quark SoC x1000
...
\item 100\% Code-Kompatibilität zu Arduino Leonardo
\item 100\% Code-Kompatibilität zu Beaglebone Black
\item recht teuer - Entwicklerkit 149 \euro - final 60 bis 80 \euro
\item \emph{Mattias' Meinung:} Tolles Konzept, hohe Leistung, aber hoher Preis und große Platine
\end{itemize}
\end{frame}

\begin{frame}
\frametitle{Arduino Yún}
...
\item 100\% Code-Kompatibilität zu Arduino Leonardo
\item OpenWRT basierte Distribution für die MIPS-Seite
\item Straßenpreis 60 bis 75 \euro - kann parallel als WLAN Access Point genutzt werden
\item \emph{Mattias' Meinung:} Tolles Konzept, passende Leistung, sollte etwas günstiger sein
\item \emph{Der Clou:} Die Software - einfacher geht es nicht!
\end{itemize}
\end{frame}

\begin{frame}
\frametitle{Arduino Yún - Software-Integration}
...
\begin{itemize}
\item Bridge-Bibliothek für Zugriff auf viele Netzwerkfunktionen aus Arduino-Sketches - nur eine Programmiersprache nötig!
\item ,,Mailbox'' (toter Briefkasten) für den Datenaustausch zwischen Microcontroller und Microprocessor
\item Integration von Temboo - abstrahiert verschiedene Webdienste
\item Voller Paketumfang von OpenWRT
\item Einfaches Webinterface für die Einrichtung - Luci bekannt von OpenWRT
\end{itemize}
\end{frame}

\begin{frame}
\frametitle{Arduino Yún - Twitter-Beispiel}
  \begin{center}
...
\begin{frame}
\frametitle{Arduino Yún - Tweets ganz praktisch}
  \begin{center}
    \includegraphics[width=9cm]{photos/tweet.png}
  \end{center}

Wozu? ,,Horst von Forst'', unser Weihnachtsbaum twitterte, wenn er zuwenig Wasser hatte. Der vollständige Code und Erklärungen zum verwendeten Sensor unter: http://www.arduino-hausautomation.de/
\end{frame}
...
\item Tendenziell Preise auf Level von Atmega32u4 oder Atmega328P - kleine Klone für weniger als 5 \euro\  möglich
\item \textbf{Achtung!} 3,3 Volt
\item \textbf{Achtung!} kein DIL Package des Controllers erhältlich
\end{itemize}
...
Im Laufe der Jahre wurden von Arduino immer mehr Microcontroller-Familien unterstützt, nicht nur kleine Sprünge wie von Atmega168 auf Atmega328, sondern mehrere Brüche. Offizielle Familien:
\newline

\begin{itemize} 
\item Atmega328: Arduino Uno, Pro Mini
\item Atmega32u4: Leonardo, Yún, Micro
...
\begin{frame}
\frametitle{Kompatibilitätsprobleme}
...
\item Unterschiedliche Position der Pins für SPI
\item Code zum extremen Energiesparen ist Controllerspezifisch
\item Viele Bibliotheken unterstützen bislang nur 328 und 32u4
\end{itemize}
\end{frame}
...
\begin{itemize} 
\item Kurzfristige Ablösung der 32u4 basierten Controller durch ARM
\item Fokus der Entwickler schnell auf ARM?
\item Atmega328P wird noch eine Weile bleiben - weil im DIL-Package erhältlich
\item Andere Controllerhersteller außer Atmega werden zu Arduino stoßen (Infineon?)
\item Billige Klone im Mini-Format machen Verlust der DILs wett
\item Arduino (im Sinne von Softwareplattform) wird offener und emanzipiert sich von Atmel
\end{itemize}
\end{frame}
